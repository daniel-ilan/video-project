...
function buildCollection(data) {
  ...
  for (let i = 0; i < data.collections_props.length; i++) {
    ...
    let myLi = `<li id="${"li_" +
      data.collections_props[i][0]}" class="${active_class}">
                        <a id="${"a_" +
                          data.collections_props[
                            i
                          ][0]}" class="nav-link collection-nav" href="#">${data
      .collections_props[i][1] + numOfAnimations} </a>
                    </li>`;
  ...
  if (data.animations_props != null) {
    for (let i = 0; i < data.animations_props[1].length; i++) {
      ...
      let animation = `<div class="frame_container_class">
        <div id="collectionAnimation_${data.animations_props[1][
          i
        ][0]}" class="tinyLottie tinyLottie_Brand animated_zoomIn zoomIn">
      ...
      <div class="form-check animated_zoomIn zoomIn">
            <input type="checkbox" class="form-check-input" id="animationCheck_${data
              .animations_props[1][i][0]}">
            <label class="form-check-label tinyLottieDescription" for="animationCheck_${data
              .animations_props[1][i][0]}">${data.animations_props[1][
        i
      ][1]}</label>
        </div>
    </div>`;
...
function disabledFunc(event) {
  if (
    ...
    event.currentTarget.classList.contains("disabled_svg")
  ) {
...
function buildPalette(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    let color = `<div id=${"colorDiv_" + data[i][1]} dataId="${data[
      i
    ][2]}" class=" color-wrapper brand_colors">
                  <input type="hidden"  name=${data[
                    i
                  ][1]} class="form-control" value=${data[i][0]} />
                  <div id="color_${data[i][1]}"  class="name-of-color"></div>
                  <span class="color-edit-line" data-toggle="tooltip" data-placement="right" title="${names[
                    i
                  ]}">
  ...
  if (data.event_kind == "chooseFromReadyPalette") {
    ...
    for (let i = 0; i < data.colors.length; i++) {
      ...
      for (let z = 0; z < data.colors[i][1].length; z++) {
        ...
        let colorPaletteZ = `<div id="color_${data.colors[i][1][
          z
        ][1]}" class="colorBranModal" style="background-color: ${data.colors[
          i
        ][1][z][0]}"> </div>`;
        let colorDesc = `<div id=${"colorDiv_" +
          data.colors[i][0][0]} class="ml-2 colorBranModalDiv ">
                                        <div class="colorBranModal" style="background-color: ${data
                                          .colors[i][1][z][0]}"></div>
                                      <div> ${names[z]} </div>
                                 </div>`;
      ...
      let divGrid = `<div id="gridLinePalette_${data.colors[
        i
      ][0][0]}" class="row paletteGrid_modal mb-4">
                            <div id="paletteArea_${data.colors[
                              i
                            ][0][0]}" class="col-4">
                            <div class="d-flex flex-row "> ${colorPalette}</div>
                             ...
                             <div class="col-8"> 
                             <div id="paletteDescription_${data.colors[
                               i
                             ][0][0]}" style="display: none">
    ...
    $(".paletteGrid_modal").on("click", function() {
      ...
      document.querySelectorAll(".paletteGrid_modal").forEach(function(event) {
        ...
        if (
          id == current_id &&
          event.classList.contains("paletteGrid_modal_Selected") != true
        ) {
          //add Selected class
          ...
          if (event.classList.contains("paletteGrid_modal_Selected")) {
            $("#gridLinePalette_" + id).removeClass(
              "paletteGrid_modal_Selected"
            );
          }
...
function buildVideoPage(data) {
  ...
  for (let i = 0; i < video_length; i++) {
    ...
    let myCard = `  <div id="video_${data.videos_props[
      i
    ][0]}" class="card zoomIn_card  video_sizes">
                            <div class="img_wrapper">
                                  <img id="videoImg_${data.videos_props[
                                    i
                                  ][0]}" class="card-img-top" src="${"../" +
      data.video_src +
      data.videos_props[i][0] +
      "/" +
      data.videos_props[i][2].replace(/\u200f/g, "")}" alt="Card image cap">
                            </div>
                            <div class="card-body">
                                <div>
                                <h5 id="h5_${data.videos_props[
                                  i
                                ][0]}" class="card-title p-2">${data
      .videos_props[i][1]}</h5>
                                <div id="changeNameArea_${data.videos_props[
                                  i
                                ][0]}" style="display: none">
                                    <div class="input-group flex-nowrap changeNameArea_div">
                                       <input type="text" class="form-control changeName-input" name="videoName" id="videoName_${data
                                         .videos_props[i][0]}" value="${data
      .videos_props[i][1]}">
                                       <div class="input-group-prepend changeName-btn">
                                            <span class="input-group-text saveNameChange" id="addon-wrapping_${data
                                              .videos_props[i][0]}">
                                  ...
                                  <small class="text-muted status ${className}">${data
      .videos_props[i][3]}</small>
                            </div>
                              <div class="card-footer">
                                <div class="d-flex d-flex justify-content-around">
                                    <svg id="edit_icon" class="card-icons-svg" data-vid="${data
                                      .videos_props[
                                      i
                                    ][0]}"  viewBox="0 0 16 16" fill="none"
                                    ...
                                    <svg id="record_icon" class="card-icons-svg ${classDisabled_rec}"  data-vid="${data
      .videos_props[i][0]}" viewBox="0 0 16 16" fill="none"
                                    xmlns="http://www.w3.org/2000/svg"  data-toggle="tooltip" data-placement="bottom"
                                    ...
                                    <svg id="watch_icon" class="card-icons-svg ${classDisabled_show}" data-vid="${data
      .videos_props[i][0]}" viewBox="0 0 16 16" fill="none"
                                    xmlns="http://www.w3.org/2000/svg"  data-toggle="tooltip" data-placement="bottom"
                                    ...
                                    <svg id="moreOption_icon" class="card-icons-svg" data-vid="${data
                                      .videos_props[
                                      i
                                    ][0]}" viewBox="0 0 16 16" fill="none"
                                    ...
                                    title="אפשרויות נוספות">
                                       ...
                                       <div class="dropdown-menu" data-flip="true" >
                                          <a id="more_changeName" data-vid="${data
                                            .videos_props[
                                            i
                                              ...
                                              שינוי שם </a>
                                          <a id="more_coverPic" data-vid="${data
                                            .videos_props[
                                            i
                                          ][0]}" class="dropdown-item"  href="#">
                                          ...
                                          <a id="more_download" class="dropdown-item ${classDisabled_show}" href="#">
                                              ...
                                              הורדה</a>
                                          <a id="more_delete" data-vid="${data
                                            .videos_props[
                                            i
                                          ][0]}" class="dropdown-item" href="#">
                                              <svg class="card-icons-svg" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  $("#current_page_breadcrumb").html(
    `סרטונים  <span id="videos_length_page"> ${"(" + video_length + ")"}</span>`
  );
...
function openVideo_Handler(event) {
  ...
  if (id == "more_delete") {
    ...
    let img_content = `<div id="editImage" enctype="multipart/form-data">
        ...
        <div>
                   <h5>תמונה נוכחית</h5>
           <img id="exists_image" src="${$("#videoImg_" + video_id).attr(
             "src"
           )}" class="mainAnimation coverImg" alt="your upload image" />
         </div>
            </div>`;
