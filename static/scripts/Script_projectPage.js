...
function disabledFunc(event) {
  if (
    ...
    if (
      event.currentTarget.id == "selectCollection" &&
      event.currentTarget.classList.contains("disabled")
    ) {
      $("#selectCollection").off("click", disabledFunc);
      $("#selectCollection").on("click", change_collection_handler);
...
function buildVideoPage(data) {
  ...
  for (let i = 0; i < videos_leangth; i++) {
    ...
    let myCard = `  <div id="video_${data.videos_props[
      i
    ][0]}" class="card animated_zoomIn zoomIn video_sizes">
                            <div class="img_wrapper">
                                  <img class="card-img-top" src="${"../" +
                                    data.video_src +
                                    data.videos_props[i][0] +
                                    "/" +
                                    data.videos_props[i][2].replace(
                                      /\u200f/g,
                                      ""
                                    )}" alt="Card image cap">
                            </div>
                            <div class="card-body">
                            <div class="flex-wrap">
                                <h5 class="card-title p-2">${data.videos_props[
                                  i
                                ][1]}</h5>
  ...
  const initial = `   
       <div class="row h-5 container-fluid mr-4 pt-2">
        <h1 id="pageTitleH" class="mr-auto pt-1 pb-3 ">סרטונים  <span id="videos_length_page"> ${"(" +
          videos_leangth +
          ")"}</span></h1>
    </div>
